...
const ClosePendingList: React.FC = () => {
...
const [cancelLoading, setCancelLoading] = useState(false);
  ...
  // ✅ Enrichment (same as ConfirmPendingList)
const enrichBookings = async (bookings: Order[]): Promise<Order[]> => {
  return Promise.all(
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get<ApiResponse>('/order/status/confirmed');
        let bookings: Order[] = response.data.data || [];
  ...
  const handleView = (order: Order) => {
    navigate(`/orders/view/close-pending-order/${order.bookingId}`, {
  ...
  const handleClose = (order: Order) => {
    navigate(`/orders/close/close-pending-order/${order.bookingCode}`, {
      state: { userName: order.userName, bookingCode: order.bookingCode }
    });
  };
  ...
  const handleEdit = (order: Order) => {
    navigate(`/orders/edit/close-pending-order/${order.bookingId}`, {
      state: { order }
    });
  };
    ...
    if (!query.trim()) {
      // reload default API
      try {
        const res = await axiosInstance.get('/order/status/confirmed');
        const bookings: Order[] = res.data.data || [];
    ...
    try {
      ...
      const pendingRes = await axiosInstance.get('/order/status/confirmed');
      const pendingBookings: Order[] = pendingRes.data?.data || [];
  ...
  return (
    <PageLayout>
      <div className="py-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-4">List Close Pending Order</h1>
        <div className="mb-4">
          <AlertContainer />
          {/* <SearchBar
            value={searchKeyword}
...
export default ClosePendingList;
