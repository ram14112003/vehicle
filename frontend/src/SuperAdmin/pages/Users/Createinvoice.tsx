...
//   return d;
// };
const effectiveCompanyId: string = selectedCompanyId || companyId || "";
...
useEffect(() => {
  ...
  useEffect(() => {
    if (formData.pickupPoint && effectiveCompanyId) {
      let type = "";
      ...
      if (type) {
        fetchPackages(type, effectiveCompanyId);
      }
    }
}, [formData.pickupPoint, effectiveCompanyId]);
...
useEffect(() => {
  ...
  // 🔥 ALSO refresh vehicle package details
  if (formData.carType) {
    const selectedCarType = carTypes.find(
      (car) => car.vehicleType === formData.carType
    );

    if (selectedCarType && formData.pickupPoint) {
      fetchPackageDetails(
        selectedCarType.vehicleTypeId,
  ...
  const fetchPackageDetails = async (vehicleTypeId: string, pickupPoint: string, effectiveCompanyId: string) => {
 ...
 const fetchPackages = async (type: string, companyIdParam?: string) => {
  ...
  try {
    const res = await axiosInstance.get<PackageDataResponse>(
      ...
      {
        params: {
          companyId: companyIdParam || companyId, // 🔥 FIX
          packageType: type
        }
      }
    );
...
const behalfOfName = formData.behalfOfPerson || null;
    const behalfOfPhone =
...
// }
  ...
  onChange={(date) => {
    ...
    // ✅ if user selects today with a past/early time, force it to minTime
    const minT = getMinSelectableTime(date);
    const fixedDate = minT && date < minT ? minT : date;

    setPickupDate(fixedDate);
   setTimeout(() => {
  const minTime = getMinSelectableTime(date);
...
<div>
  ...
  {/* <input
    ...
    value={
      showManagerDropdown
        ? managerSearch
        : selectedManager
        ? `${selectedManager.username}${selectedManager.email ? ` (${selectedManager.email})` : ""}`
        : managerSearch
    }
    onChange={(e) => {
      setManagerSearch(e.target.value);
      setShowManagerDropdown(true);
      setManagerUserId(""); // optional: typing again clears selection
    }}
    onFocus={() => {
      setShowManagerDropdown(true);
      // if already selected, show search as empty so user can type again
  ...
  //   setManagerEmail(value);
    ...
    // if (value && !value.toLowerCase().endsWith("@danfoss.com")) {
    //   setManagerEmailError("Danfoss users must use @danfoss.com email only.");
    // } else {
    //   setManagerEmailError("");
    // }
//     if (value && !value.toLowerCase().endsWith("@gmail.com")) {
//   setManagerEmailError("Only Gmail (name@gmail.com) is allowed.");
// } else {
//   setManagerEmailError("");
// }
//   }} 
...
/>
  {/* {managerEmailError && (
  <p className="text-red-500 text-sm mt-1">{managerEmailError}</p>
)} */}
  {/* Dropdown */}
  {/* {showManagerDropdown && !loadingManagers && managers.length > 0 && (
    <div className="absolute z-30 mt-1 w-full bg-white border rounded shadow max-h-52 overflow-y-auto">
      {filteredManagers.length === 0 ? (
        ...
        filteredManagers.map((m) => (
          <button
            ...
            onClick={() => {
              setManagerUserId(m.userId);
              setManagerSearch(`${m.username}${m.email ? ` (${m.email})` : ""}`);
              setShowManagerDropdown(false);
            }}
          >
