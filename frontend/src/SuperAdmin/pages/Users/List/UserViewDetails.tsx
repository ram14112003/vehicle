...
useEffect(() => {
  ...
  // 👇 Move fetchUserDetails function here (component level)
  const fetchUserDetails = async () => {
    try {
      const response = await axiosInstance.get<UserDetailsResponse>(`/user/${userId}`);
      const { addressList = [], ...userData } = response.data.data;
...
const handleUpdateUser = async (e: React.FormEvent) => {
  ...
  const isDanfoss = selectedCompany?.companyName
    ?.toLowerCase()
    .includes("danfoss");
  ...
  if (isDanfoss) {
    ...
    if (!emailLower.endsWith("@danfoss.com")) {
      showToast(
        "Danfoss company users must use @danfoss.com email only.",
        "error"
      );
      return;
    }
            ...
            <div className="mt-4">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {['Address', 'Present Address', 'Pin Code', 'City', 'State', 'Country', 'Action'].map((head) => (
                      <th
                        key={head}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  <tr>
                    <td className="px-2 py-4">{user.userAddress || 'N/A'}</td>
                    <td className="px-2 py-4">{user.presentAddress || 'N/A'}</td>
